...
// Áreas existentes; si la propiedad aún no tiene, se arranca con la principal.
const areasIniciales = Array.isArray(props.propiedad.areas) && props.propiedad.areas.length
    ? props.propiedad.areas.map((a, i) => ({
        nombre: a.nombre ?? '',
        renta: a.renta ?? '',
        principal: i === 0,
    }))
    : [{ nombre: 'Principal', renta: '', principal: true }];
